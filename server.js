...
function publicConfig() {
  ...
  return {
    ...
    autoCycleCooldownSeconds: numberFrom(process.env.AUTO_CYCLE_COOLDOWN_SECONDS, 60),
...
function sanitizeStartPayload(payload = {}) {
  ...
  return {
    ...
    autoCycleCooldownSeconds: Math.max(0, Math.floor(numberFrom(payload.autoCycleCooldownSeconds, numberFrom(process.env.AUTO_CYCLE_COOLDOWN_SECONDS, 60)))),
...
function summarizeRun(run, reason = run?.reason || 'manual') {
  ...
  return {
    ...
    autoCycleCooldownSeconds: Number(snapshot.autoCycleCooldownSeconds ?? run.autoCycleCooldownSeconds ?? run.config?.autoCycleCooldownSeconds ?? 60),
  ...
  bot.on('balance_update', (event) => {
    ...
    void persistRunState(bot, {
      ...
      extraPatch: {
        ...
        autoCycleCooldownSeconds: event.autoCycleCooldownSeconds,
...
async function startFreshRun(payload = {}) {
  ...
  const runDoc = await runStore.createRun({
    ...
    autoCycleCooldownSeconds: initialSnapshot.autoCycleCooldownSeconds,
